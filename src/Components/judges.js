      ...
      <Slider {...settings}>
        {data.map((d) => (
          <div key={d.name} className="bg-white h-[450px] text-black rounded-xl">
            <div className='h-56 bg-green-700 flex justify-center items-center rounded-t-xl'>
            ...
            <div className="flex flex-col items-center justify-center gap-4 p-4">
              ...
              <p className="text-center">{d.description}</p>
               <button className='bg-green-700 text-white text-lg px-6 py-1 rounded-xl'>Read More</button>
            </div>
          </div>
        ))}
      </Slider>
